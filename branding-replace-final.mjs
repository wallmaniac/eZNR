// branding-replace-final.mjs
import { readFileSync, writeFileSync } from 'fs';

const FILE = 'src/app/dashboard/settings/page.js';
const content = readFileSync(FILE, 'utf8');

const sIdx = content.indexOf("<h3 style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>");
const bIdx = content.lastIndexOf("<hr", sIdx);
if (sIdx === -1 || bIdx === -1) throw new Error("Could not find start hr/h3");

const eIdx = content.indexOf("Spremi branding i firmu'", sIdx);
const eEnd = content.indexOf("</button>", eIdx) + 9;
if (eIdx === -1 || eEnd < 9) throw new Error("Could not find end button");

const before = content.substring(0, bIdx);
const after = content.substring(eEnd);
