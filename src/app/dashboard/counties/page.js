...
export default function CountiesPage() {
  ...
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.COUNTIES || 'counties', id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems2').replace('{0}', selectedIds.size))) {
    ...
    <><DialogRenderer /><div className="animate-fadeIn">
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {country === 'HR' ? t('newCountyHR') : t('newCountyBA')}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
