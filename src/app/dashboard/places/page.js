...
export default function PlacesPage() {
  ...
  const handleSave = () => { if (!formData.naziv) return; if (editingId) update(COLLECTIONS.PLACES, editingId, formData); else create(COLLECTIONS.PLACES, formData); setShowForm(false); loadData(); showFlash(); };
  const handleDelete = async (id) => { if (await confirm(bs ? 'Obrisati?' : 'Delete?')) { remove(COLLECTIONS.PLACES, id); loadData(); } };
  const handleDeleteSelected = async () => { if (selectedIds.size === 0) return; if (await confirm(bs ? `Obrisati ${selectedIds.size} stavki?` : `Delete ${selectedIds.size} items?`)) { for (const id of selectedIds) remove(COLLECTIONS.PLACES, id); setSelectedIds(new Set()); loadData(); } };

  const menuItemSt = { display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', background: 'none', border: 'none', cursor: 'pointer', width: '100%', fontSize: '0.85rem', fontWeight: 500, color: 'var(--text)', textAlign: 'left', transition: 'background 0.12s' };

  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="📍" title={t('places')} />
      {showForm && (
          ...
          <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                <label className="form-label">{bs ? 'Poštanski broj' : 'Postal code'}</label>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {bs ? 'Nova lokacija' : 'New Location'}</button>
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('postBroj')} onClick={() => toggleSort('postBroj')}>{bs ? 'Poštanski broj' : 'Postal code'}{sortIcon('postBroj')}</th>
        </tr></thead>
