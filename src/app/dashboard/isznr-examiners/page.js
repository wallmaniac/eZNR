...
export default function ISZNRExaminersPage() {
  ...
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.ISZNR_EXAMINERS || 'isznr_examiners', id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems17').replace('{0}', selectedIds.size))) {
      for (const id of selectedIds) remove(COLLECTIONS.ISZNR_EXAMINERS || 'isznr_examiners', id);
