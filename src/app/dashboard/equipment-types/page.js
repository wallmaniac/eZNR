...
export default function EquipmentTypesPage() {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (await confirm(t('deleteItems7').replace('{0}', selectedIds.size))) {
  ...
  const handleDelete = async (id) => { const ok = await confirm(t('obrisati')); if (ok) { remove(COLLECTIONS.EQUIPMENT_TYPES, id); loadData(); } };
      ...
      <div className="animate-fadeIn">
        ...
        <div className="card"><div className="card-body">
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            <button className="btn btn-primary btn-sm" onClick={handleNew}>+ {t('noviTipOpreme')}</button>
            <SavedFlash />
            {selectedIds.size> 0 && (
