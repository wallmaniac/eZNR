...
export default function AuthorizedCompaniesPage() {
  ...
  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="🏗️" title={t('authorizedCompanies')} />
      {showForm && (
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <tbody>{sorted.length === 0 ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('noRecords')}</td></tr> : sorted.map(i => (
            <tr key={i.id} onClick={() => handleEdit(i)} style={{ cursor: 'pointer', transition: 'background 0.12s' }} onMouseEnter={e => e.currentTarget.style.background='var(--bg-table-row-hover)'} onMouseLeave={e => e.currentTarget.style.background=''}>
              ...
              <td><code>{i.rješenjeBroj}</code></td>
              <td>{i.adresa}</td>
              <td>{i.tel}</td>
            </tr>
          ))}</tbody></table></div>
