          ...
          <div className="card-body" style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative' }}>
              <button className="btn btn-dark btn-sm" onClick={() => {
                const el = document.getElementById('group-menu-nr1');
                if (el) el.style.display = el.style.display === 'none' ? 'block' : 'none';
              }}>{lang === 'bs' ? 'Grupne akcije' : 'Group actions'} ▼</button>
              <div id="group-menu-nr1" className="dropdown-menu" style={{ display: 'none', right: 0, top: 'calc(100% + 4px)', minWidth: 230, zIndex: 999 }}>
                ...
                <button className="dropdown-item" disabled={selectedIds.size === 0} style={{ opacity: selectedIds.size === 0 ? 0.5 : 1 }} onClick={() => { document.getElementById('group-menu-nr1').style.display = 'none'; window.print(); }}>🖨️ {lang === 'bs' ? 'Ispiši odabrane' : 'Print selected'}</button>
                <div className="dropdown-divider" />
                <button className="dropdown-item" disabled={selectedIds.size === 0} style={{ color: selectedIds.size > 0 ? 'var(--danger)' : 'var(--text-muted)', opacity: selectedIds.size === 0 ? 0.5 : 1 }} onClick={() => { document.getElementById('group-menu-nr1').style.display = 'none'; handleDeleteSelected(); }}>🗑️ {lang === 'bs' ? `Obriši odabrane (${selectedIds.size})` : `Delete selected (${selectedIds.size})`}</button>
              </div>
            </div>
