...
export default function OrgGroupsPage() {
  ...
  const handleSave = () => { if (!formData.naziv) return; const col = COLLECTIONS.ORG_GROUPS || 'org_groups'; if (editingId) update(col, editingId, formData); else create(col, formData); setShowForm(false); loadData(); };
  const handleDelete = (id) => { if (confirm(lang === 'bs' ? 'Obrisati?' : 'Delete?')) { remove(COLLECTIONS.ORG_GROUPS || 'org_groups', id); loadData(); } };
  return (
