...
export default function OrgGroupsPage() {
  ...
  const handleSave = () => {
    ...
    const col = COLLECTIONS.ORG_GROUPS || 'org_groups';
    if (editingId) update(col, editingId, formData); else create(col, formData);
    setShowForm(false); loadData(); showFlash();
  };
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.ORG_GROUPS || 'org_groups', id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems22').replace('{0}', selectedIds.size))) {
      for (const id of selectedIds) remove(COLLECTIONS.ORG_GROUPS || 'org_groups', id);
  ...
  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="🗺️" title={t('orgUnitGroups')} />
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {t('newGroup')}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
