...
export default function ExtraFieldsPage() {
  ...
  const handleSave = () => {
    ...
    const col = COLLECTIONS.EXTRA_FIELDS || 'extra_fields';
    if (editingId) update(col, editingId, formData); else create(col, formData);
    setShowForm(false); loadData(); showFlash();
  };
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.EXTRA_FIELDS || 'extra_fields', id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems10').replace('{0}', selectedIds.size))) {
      for (const id of selectedIds) remove(COLLECTIONS.EXTRA_FIELDS || 'extra_fields', id);
  ...
  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="🗺️" title={t('extraFields')} />
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{(t('tip'))}</label><input className="form-input" value={formData.tip} onChange={e => setFormData({ ...formData, tip: e.target.value })} /></div>
        </div>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {t('newField')}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('tip')} onClick={() => toggleSort('tip')}>{(t('tip'))}{sortIcon('tip')}</th>
        </tr></thead>
          <tbody>{sorted.length === 0 ? <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('noRecords')}</td></tr> : sorted.map(i => (
            <tr key={i.id} onClick={() => handleEdit(i)} style={{ cursor: 'pointer', transition: 'background 0.12s' }}>
              ...
              <td><span className="badge badge-info">{i.tip}</span></td>
            </tr>
          ))}</tbody></table></div>
      </div></div>
    </div></>
