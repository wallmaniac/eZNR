    ...
    <div className="animate-fadeIn">
      ...
      <div className="card"><div className="card-body">
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th>{t('orgUnit')}</th>
          <th>{t('strucnaSprema')}</th>
          <th>{t('posebniUvjeti')}</th>
        </tr></thead><tbody>
            {sorted.length === 0 ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('nemaRadnihMjesta')}</td></tr> : sorted.map((w, idx) => (
              <tr key={w.id} onClick={() => router.push(`/dashboard/workplaces?openItem=${w.id}&returnTo=/dashboard/workplace-list`)} style={{ cursor: 'pointer', transition: 'background 0.12s' }}>
                ...
                <td>{w.posebniUvjetiRada ? <span className="badge badge-danger">{t('da')}</span> : <span className="badge badge-success">{t('ne')}</span>}</td>
              </tr>
            ))}
          </tbody></table></div>
      </div></div>
