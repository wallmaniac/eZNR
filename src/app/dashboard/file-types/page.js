...
export default function FileTypesPage() {
  ...
  const handleSave = () => {
    ...
    const col = COLLECTIONS.FILE_TYPES || 'file_types';
    if (editingId) update(col, editingId, formData); else create(col, formData);
    setShowForm(false); loadData(); showFlash();
  };
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.FILE_TYPES || 'file_types', id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems11').replace('{0}', selectedIds.size))) {
      for (const id of selectedIds) remove(COLLECTIONS.FILE_TYPES || 'file_types', id);
  ...
  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="🗺️" title={t('fileTypes')} />
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{(t('extension'))}</label><input className="form-input" value={formData.ekstenzija} onChange={e => setFormData({ ...formData, ekstenzija: e.target.value })} /></div>
        </div>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {t('newFileType')}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('ekstenzija')} onClick={() => toggleSort('ekstenzija')}>{(t('extension1'))}{sortIcon('ekstenzija')}</th>
        </tr></thead>
          <tbody>{sorted.length === 0 ? <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('noRecords')}</td></tr> : sorted.map(i => (
            <tr key={i.id} onClick={() => handleEdit(i)} style={{ cursor: 'pointer', transition: 'background 0.12s' }}>
              ...
              <td><span className="badge badge-info">{i.ekstenzija}</span></td>
            </tr>
          ))}</tbody></table></div>
      </div></div>
    </div></>
