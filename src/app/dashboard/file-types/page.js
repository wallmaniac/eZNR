...
export default function FileTypesPage() {
  ...
  const handleDelete = async (id) => { if (await confirm(bs ? 'Obrisati?' : 'Delete?')) { remove(COLLECTIONS.FILE_TYPES || 'file_types', id); loadData(); } };
    ...
    <><DialogRenderer /><div className="animate-fadeIn">
      ...
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{(bs ? 'Ekstenzija' : 'Extension')}</label><input className="form-input" value={formData.ekstenzija} onChange={e => setFormData({ ...formData, ekstenzija: e.target.value })} /></div>
        </div>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {bs ? 'Novi tip datoteke' : 'New File Type'}</button>
          <SavedFlash />
          {selectedIds.size > 0 && (
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('ekstenzija')} onClick={() => toggleSort('ekstenzija')}>{(bs ? 'Ekstenzija' : 'Extension')}{sortIcon('ekstenzija')}</th>
        </tr></thead>
