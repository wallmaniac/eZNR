...
export default function FileTypesPage() {
  ...
  const [formData, setFormData] = useState({ naziv: '', ekstenzija: '' });
  const loadData = useCallback(() => { setItems(getAll(COLLECTIONS.FILE_TYPES || 'file_types')); }, []);
  useEffect(() => {
      loadData();
    ...
    <><DialogRenderer /><div className="animate-fadeIn">
      ...
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 450 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{lang === 'bs' ? 'Ekstenzija' : 'Extension'}</label><input className="form-input" value={formData.ekstenzija} onChange={e => setFormData({ ...formData, ekstenzija: e.target.value })} placeholder=".pdf" /></div>
        </div>
      ...
      <div className="card"><div className="card-body">
        <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}><button className="btn btn-primary btn-sm" onClick={handleNew}>+ {lang === 'bs' ? 'Novi tip datoteke' : 'New File Type'}</button></div>
        <div className="data-table-wrapper"><table className="data-table"><thead><tr><th style={{ width: 120 }}>{t('actions')}</th><th>{t('name')}</th><th>{lang === 'bs' ? 'Ekstenzija' : 'Extension'}</th></tr></thead>
          <tbody>{items.length === 0 ? <tr><td colSpan={3} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('noRecords')}</td></tr> : items.map(i => (
            <tr key={i.id} onClick={() => handleEdit(i)} style={{ cursor: 'pointer', transition: 'background 0.12s' }} onMouseEnter={e => e.currentTarget.style.background='var(--bg-table-row-hover)'} onMouseLeave={e => e.currentTarget.style.background=''}><td onClick={e => e.stopPropagation()}><div style={{ display: 'flex', gap: 4 }}><button className="btn btn-primary btn-sm" onClick={() => handleEdit(i)}>✏️</button><button className="btn btn-ghost btn-sm" onClick={() => handleDelete(i.id)} style={{ color: 'var(--danger)' }}>🗑️</button></div></td><td style={{ fontWeight: 600 }}>{i.naziv}</td><td><code>{i.ekstenzija}</code></td></tr>
          ))}</tbody></table></div>
      </div></div>
    </div></>
  );
