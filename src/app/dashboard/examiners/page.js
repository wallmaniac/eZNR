...
export default function ExaminersPage() {
  ...
  const handleDelete = async (id) => { if (await confirm(bs ? 'Obrisati?' : 'Delete?')) { remove(COLLECTIONS.EXAMINERS, id); loadData(); } };
    ...
    <><DialogRenderer /><div className="animate-fadeIn">
      ...
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{(bs ? 'Zvanje' : 'Title')}</label><input className="form-input" value={formData.zvanje} onChange={e => setFormData({ ...formData, zvanje: e.target.value })} /></div>
        </div>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {bs ? 'Novi ispitivac' : 'New Examiner'}</button>
          <SavedFlash />
          {selectedIds.size > 0 && (
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('zvanje')} onClick={() => toggleSort('zvanje')}>{(bs ? 'Zvanje' : 'Title')}{sortIcon('zvanje')}</th>
        </tr></thead>
