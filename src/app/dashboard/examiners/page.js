...
export default function ExaminersPage() {
  ...
  const loadData = useCallback(() => { setItems(getAll(COLLECTIONS.EXAMINERS)); }, []);
  useEffect(() => {
    loadData();
  ...
  const { sorted, toggleSort, sortIcon, thStyle } = useSortedList(items, 'ime');
  ...
  const handleSave = () => {
    ...
    const col = COLLECTIONS.EXAMINERS;
    if (editingId) update(col, editingId, formData); else create(col, formData);
    setShowForm(false); loadData(); showFlash();
  };
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.EXAMINERS, id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems9').replace('{0}', selectedIds.size))) {
      for (const id of selectedIds) remove(COLLECTIONS.EXAMINERS, id);
  ...
  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="🗺️" title={t('examiners')} />
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{(t('zvanje'))}</label><input className="form-input" value={formData.zvanje} onChange={e => setFormData({ ...formData, zvanje: e.target.value })} /></div>
        </div>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {t('noviIspitivac')}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('zvanje')} onClick={() => toggleSort('zvanje')}>{(t('zvanje'))}{sortIcon('zvanje')}</th>
        </tr></thead>
          <tbody>{sorted.length === 0 ? <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('noRecords')}</td></tr> : sorted.map(i => (
            <tr key={i.id} onClick={() => handleEdit(i)} style={{ cursor: 'pointer', transition: 'background 0.12s' }}>
                  ...
                  {actionMenuId === i.id && (<>
                    ...
                    <div onMouseDown={(e) => e.preventDefault()} style={{ position: 'fixed', top: menuPos.top, bottom: menuPos.bottom, left: menuPos.left, zIndex: 9999, userSelect: 'none', WebkitUserSelect: 'none', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', boxShadow: '0 8px 32px rgba(0,0,0,0.28)', minWidth: 200, maxHeight: menuPos.maxH, overflowY: 'auto' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid var(--border-light)' }}>
                        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{t(i.ime?.trim()) || i.ime}</span>
              ...
              <td style={{ fontWeight: 600 }}>{t(i.ime?.trim()) || i.ime}</td>
              <td>{i.zvanje}</td>
            </tr>
          ))}</tbody></table></div>
      </div></div>
