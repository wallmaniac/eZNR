...
export default function ISZNRPartiesPage() {
  ...
  const handleDelete = async (id) => { if (await confirm(bs ? 'Obrisati?' : 'Delete?')) { remove(COLLECTIONS.ISZNR_PARTIES, id); loadData(); } };
    ...
    <><DialogRenderer /><div className="animate-fadeIn">
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {bs ? 'Nova stranka' : 'New Party'}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
