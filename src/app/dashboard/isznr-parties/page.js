...
export default function ISZNRPartiesPage() {
  ...
  const [formData, setFormData] = useState({ naziv: '', oib: '', kontaktOsoba: '', adresa: '', telefon: '', email: '' });
  ...
  const handleNew = () => { setFormData({ naziv: '', oib: '', kontaktOsoba: '', adresa: '', telefon: '', email: '' }); setEditingId(null); setShowForm(true); };
  const handleEdit = (item) => { setFormData({ ...item }); setEditingId(item.id); setShowForm(true); };
  const handleSave = () => {
    if (!formData.naziv) return;
    const col = COLLECTIONS.ISZNR_PARTIES;
    if (editingId) update(col, editingId, formData); else create(col, formData);
    setShowForm(false); loadData(); showFlash();
  };
  const handleDelete = async (id) => { if (await confirm(t('obrisati'))) { remove(COLLECTIONS.ISZNR_PARTIES, id); loadData(); } };
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    if (await confirm(t('deleteItems19').replace('{0}', selectedIds.size))) {
      for (const id of selectedIds) remove(COLLECTIONS.ISZNR_PARTIES, id);
  ...
  return (
    <><DialogRenderer /><div className="animate-fadeIn">
      <PageHeader icon="🗺️" title={t('parties')} />
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label className="form-label">{'OIB'}</label><input className="form-input" value={formData.oib} onChange={e => setFormData({ ...formData, oib: e.target.value })} /></div>
        </div>
      ...
      <div className="card"><div className="card-body" style={{ padding: 0 }}>
        <div className="scrollable-toolbar" style={{ padding: '8px 16px', display: 'flex', gap: 14, alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ flexShrink: 0, height: 38 }} onClick={handleNew}>+ {t('newParty')}</button>
          <SavedFlash />
          {selectedIds.size> 0 && (
        ...
        <div className="data-table-wrapper"><table className="data-table"><thead><tr>
          ...
          <th style={thStyle('oib')} onClick={() => toggleSort('oib')}>{'OIB'}{sortIcon('oib')}</th>
        </tr></thead>
