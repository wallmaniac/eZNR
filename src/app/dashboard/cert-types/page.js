...
export default function CertTypesPage() {
  ...
  const [formData, setFormData] = useState({ naziv: '', oznaka: '', trajanjeMjeseci: 24 });
  ...
  const handleDeleteSelected = async () => {
    ...
    if (await confirm(t('deleteItems1').replace('{0}', selectedIds.size))) {
  ...
  const handleNew = () => { setFormData({ naziv: '', oznaka: '', trajanjeMjeseci: 24 }); setEditingId(null); setShowForm(true); };
  ...
  const handleDelete = async (id) => { const ok = await confirm(t('obrisati')); if (ok) { remove(COLLECTIONS.CERT_TYPES, id); loadData(); } };
  ...
  return (
    <>
      ...
      <div className="animate-fadeIn">
        <PageHeader icon="🎓" title={t('certTypes')} subtitle={`${sorted.length} ${t('zapisa')}`} />
        {showForm && (
            ...
            <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-body">
                ...
                <div className="form-grid-2">
                  <div className="form-group"><label className="form-label">{t('oznaka')}</label><input className="form-input" value={formData.oznaka} onChange={e => setFormData({ ...formData, oznaka: e.target.value })} /></div>
                  <div className="form-group"><label className="form-label">{t('trajanjeMjeseci')}</label><input className="form-input" type="number" value={formData.trajanjeMjeseci} onChange={e => setFormData({ ...formData, trajanjeMjeseci: Number(e.target.value) })} /></div>
                </div>
              </div>
        ...
        <div className="card"><div className="card-body">
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            <button className="btn btn-primary btn-sm" onClick={handleNew}>+ {t('noviTipUvjerenja')}</button>
            <SavedFlash />
            {selectedIds.size> 0 && (
          ...
          <div className="data-table-wrapper"><table className="data-table"><thead><tr>
            ...
            <th onClick={() => toggleSort('oznaka')} style={thStyle('oznaka')}>{t('oznaka')}{sortIcon('oznaka')}</th>
            <th onClick={() => toggleSort('trajanjeMjeseci')} style={thStyle('trajanjeMjeseci')}>{t('trajanje')}{sortIcon('trajanjeMjeseci')}</th>
          </tr></thead>
            <tbody>{sorted.length === 0 ? <tr><td colSpan={5} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>{t('noRecords')}</td></tr> : sorted.map(i => (
              <tr key={i.id} onClick={() => handleEdit(i)} style={{ cursor: 'pointer' }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {actionMenuId === i.id && (
                        ...
                        <div data-menu onMouseDown={(e) => e.preventDefault()} style={{ position: 'fixed', top: menuPos.top, bottom: menuPos.bottom, left: menuPos.left, zIndex: 9999, userSelect: 'none', WebkitUserSelect: 'none', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', boxShadow: '0 8px 32px rgba(0,0,0,0.28)', minWidth: 220, maxHeight: menuPos.maxH, overflowY: 'auto' }}>
                            <button onClick={() => { setActionMenuId(null); handleEdit(i); }} className="dropdown-item">✏️ Otvori</button>
                            <div style={{ borderTop: '1px solid var(--border-light)', margin: '2px 0' }} />
                            <button onClick={() => { setActionMenuId(null); handleDelete(i.id); }} className="dropdown-item text-danger">🗑️ Izbriši</button>
                        </div>
                        </>
                    )}
                  </div>
