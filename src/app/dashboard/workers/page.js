...
const openBase64InTab = (log) => {
    if (!log?.docData) return;
    const [meta, b64] = log.docData.split(',');
    const isPdf = meta.includes('pdf');
    const isImg = meta.includes('image');
    if (isPdf || isImg) {
        const w = window.open('');
        if (w) {
            w.document.write(`<iframe src="${log.docData}" frameborder="0" style="border:0; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%;" allowfullscreen></iframe>`);
            w.document.title = log.docName || log.ime || 'Dokument';
            return;
        }
    }
    ...
    const blob = new Blob([ab], { type: log.docType || 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = log.docName || 'dokument.bin';
    a.click();
