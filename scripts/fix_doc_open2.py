...
for filepath in files:
    ...
    # 2. Replace old attachment display span with clickable button
    # Old: <span style={{ fontSize: '0.85rem', color: 'var(--info)' }}>📎 {formData.docName}</span>
    ...
    if OLD_SPAN in content:
        ...
        # Already has openDoc button or different format - try simpler replace
        old_remove_only = "onClick={(e) => { e.preventDefault(); setFormData(p => ({ ...p, docName: '', docData: '' })); }} style={{ marginLeft: 'auto', color: 'var(--danger)' }}>\u2715 {lang === 'bs' ? 'Ukloni' : 'Remove'}</button>"
        if old_remove_only in content and 'openDoc' not in content.split(old_remove_only)[0].split('{/* \u2550\u2550\u2550 Document Upload')[-1]:
            print(f"  [{name}] INFO: Different format - skipping UI upgrade")
        else:
            print(f"  [{name}] Already upgraded or different layout")
