// branding-replace.mjs — run with: node branding-replace.mjs
// Replaces encoding-corrupted branding section with clean UTF-8 content
...
// Find start (line 791 in 1-indexed = index 790) and end (line 1038 = index 1037)
const START = 790;  // 0-indexed, inclusive
const END   = 1037; // 0-indexed, inclusive
...
const newSection = `
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', marginBottom: 24 }}>
                  {lang === 'bs'
                    ? 'Prilagodite izgled PDF izvjeStaja i korisnickog sucelja prema vizualnom identitetu vaSe firme.'
                ...
                <div style={{ borderRadius: 16, background: 'var(--bg-input)', border: '1px solid var(--border)', marginBottom: 16, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 10 }}>{lang === 'bs' ? 'Koristi se za zaglavlje, linije i naglaSene elemente.' : 'Used for header lines, badges and accents.'}</div>
                    ...
                    {/* === LOGO POSITION & SIZE === */}
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 10, color: 'var(--text)' }}>{lang === 'bs' ? 'Logo – pozicija i velicina' : 'Logo Position & Size'}</div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', whiteSpace: 'nowrap', minWidth: 60 }}>{lang === 'bs' ? 'Velicina:' : 'Size:'}</span>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 8 }}>{lang === 'bs' ? 'Opcionalni tekst ispod zaglavlja na svim PDF izvjeStajima.' : 'Optional text below the header on all PDF reports.'}</div>
                      <input type="text" value={headerText}
                        onChange={e => { setHeaderText(e.target.value); setDirty('company'); }}
                        placeholder={lang === 'bs' ? 'Npr. "ZaStita na radu i poZaru"' : 'e.g. "Safety & Health Division"'}
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                        <div style={{ fontWeight: 600, fontSize: '0.82rem', color: 'var(--text)' }}>{lang === 'bs' ? 'Vodeni Zig' : 'Watermark'}</div>
                        <button
                          onClick={() => { setWmEnabled(e => !e); setDirty('company'); }}
                          title={wmEnabled ? 'Iskljuci' : 'Ukljuci'}
                        ...
                        >{wmEnabled ? (lang === 'bs' ? 'Ukljucen' : 'ON') : (lang === 'bs' ? 'Iskljucen' : 'OFF')}</button>
                      </div>
                      ...
                      {wmEnabled && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                          {/* Content type */}
                          <div>
                            <div style={{ fontSize: '0.73rem', color: 'var(--text-muted)', marginBottom: 6, fontWeight: 600 }}>{lang === 'bs' ? 'Sadrzaj:' : 'Content:'}</div>
                          ...
                          {/* Size */}
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <span style={{ fontSize: '0.73rem', color: 'var(--text-muted)', minWidth: 100, whiteSpace: 'nowrap' }}>{lang === 'bs' ? 'Velicina:' : 'Size:'}</span>
                    ...
                    {/* === LIVE PDF PREVIEW === */}
                    <div>
                      <div style={{ fontSize: '0.73rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 10 }}>{lang === 'bs' ? 'PDF pregled uzivo' : 'Live PDF Preview'}</div>
                ...
                <div style={{ borderRadius: 16, background: 'var(--bg-input)', border: '1px solid var(--border)', overflow: 'hidden', marginBottom: 16 }}>
                  ...
                  <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    {/* === SIDEBAR COLOR === */}
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 4, color: 'var(--text)' }}>{lang === 'bs' ? 'Boja bocne trake' : 'Sidebar color'}</div>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                    ...
                    <div style={{ paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                      <div style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 12, color: 'var(--text)' }}>{lang === 'bs' ? 'Logo i tekst u bocnoj traci' : 'Sidebar Logo & Text'}</div>
                      <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 12 }}>
                        ...
                        <span style={{ fontSize: '0.82rem', color: 'var(--text)' }}>{lang === 'bs' ? 'Koristi logo firme u bocnoj traci' : 'Use company logo in sidebar'}</span>
                      </label>
                    ...
                    {/* === LIVE UI PREVIEW === */}
                    <div>
                      <div style={{fontSize:'0.73rem',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:10}}>{lang==='bs'?'Pregled uzivo':'Live Preview'}</div>
                    ...
                    {(uiPrimaryColor||uiSidebarColor||sidebarLogoEnabled)&&(
                      <button onClick={()=>{setUiPrimaryColor('');setUiSidebarColor('');setSidebarLogoEnabled(false);setSidebarText(UI_DEFAULTS.sidebarText);setDirty('company');}}
                        style={{padding:'7px 16px',borderRadius:8,border:'1px solid var(--border)',background:'transparent',color:'var(--text-muted)',cursor:'pointer',fontSize:'0.8rem',fontWeight:600,display:'flex',alignItems:'center',gap:6,width:'fit-content'}}>
